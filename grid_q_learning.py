...
class Environment(Scene):
    def setup(self):
        ...
        )  # q table for actions, state, and reward triplets
    ...
    def optimal_action(self):
        ...
        for i in range(4):
            possible_rewards.append(
                self.Q[
                    self.pos_to_state[
                        int(self.agent.position.x - self.length),
                        int(self.agent.position.y - self.length),
                    ],
                    i,
                ]
            )
    ...
    def step(self, action):
        self.move(action)
        reward = self.squares[(self.agent.position.x / self.length) - 1][
            (self.agent.position.y / self.length) - 1
        ]
        new_state = self.pos_to_state[
            int(self.agent.position.x - self.length),
            int(self.agent.position.y - self.length),
        ]
